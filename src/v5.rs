...
mod test {
    ...
    fn google_no_auth() {
        ...
        let socket =
            Socks5Stream::connect(SOCKS_PROXY_NO_AUTH_ONLY, addr, Authentication::None, None)
                .unwrap();
        google(socket);
    }
    ...
    fn auth_method_not_supported() {
        let addr = "google.com:80".to_socket_addrs().unwrap().next().unwrap();
        let err = Socks5Stream::connect(SOCKS_PROXY_PASSWD_ONLY, addr, Authentication::None, None)
            .unwrap_err();
